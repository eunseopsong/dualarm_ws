...
def calculate_target_pose(current_pose_se3, pose_init, tracker_init, tracker_current, T_robot2station, Kp_pos=0.6, Kp_rot=1.0):
    ...
    # 미세한 움직임 무시
    if np.linalg.norm(delta_vr_pos) < 0.001 and np.linalg.norm(delta_vr_rot) < 0.001:
        # 이 경우 목표 포즈는 초기 포즈와 동일해야 하지만,
        # 현재 코드 로직은 delta가 0이 되어도 target_pose가 current_pose가 아닌
        # pose_init으로 돌아가려는 경향이 있을 수 있습니다.
        # 보수적으로 delta가 작으면 그냥 현재 포즈를 목표로 설정할 수도 있으나,
        # 원본 로직을 최대한 유지하기 위해 delta가 0일 때의 동작을 따릅니다.
